...
class SRNet(paddle.nn.Layer):
    def __init__(self, upscale=4):
        ...
        self.conv1 = nn.Conv2D(1, 64, [1 ,2], stride=1, padding=0, dilation=1)
    ...
    def forward(self, x_in):
        ...
        x = paddle.reshape(x,[B, C, self.upscale * H , self.upscale * (W - 1)])
        return x
...
with paddle.no_grad():
    ...
    for ti, fn in enumerate(files_gt):
        ...
        # Run model
        batch_S1 = model_G(F.pad(val_L, (0, 1, 0, 0), mode='reflect'))

        batch_S2 = model_G(F.pad(paddle.rot90(val_L, 1, [2, 3]), (0, 1, 0, 0), mode='reflect'))
        batch_S2 = paddle.rot90(batch_S2, 3, [2, 3])

        batch_S3 = model_G(F.pad(paddle.rot90(val_L, 2, [2, 3]), (0, 1, 0, 0), mode='reflect'))
        batch_S3 = paddle.rot90(batch_S3, 2, [2, 3])

        batch_S4 = model_G(F.pad(paddle.rot90(val_L, 3, [2, 3]), (0, 1, 0, 0), mode='reflect'))
